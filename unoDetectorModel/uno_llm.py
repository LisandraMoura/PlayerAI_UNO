...
class UnoGame:
    ...
    def uno_assistant(self):
        ...
        prompt = f"Você está jogando uma partir de UNO e sua mão nessa jogada é {self.player_hand}. Dado que a pilha de descarte está com a carta {self.discard_pile}, jogue uma carta de sua mão na pilha de descarte usando a melhor estratégia para a eliminação de cartas. Lembre-se, quanto mais cartas conseguir jogar na pilha de descartes em uma jogada, melhor é, porém, tem que seguir as regras do jogo UNO, sem inventar. ##Exemplos de saídas## Output: Jogue as cartas 7 verde, 7 vermelho e 7 azul juntas. Output: Jogue a Comprar 2 Verde. ###Limite a saída em no máximo 40 caracteres, seja direto sobre a estratégia escolhida###"
        response = model.generate_content(prompt)  # Ajuste para o nome correto do método
        print(f"####Prompt enviado ao modelo###/n/n: {prompt}")
